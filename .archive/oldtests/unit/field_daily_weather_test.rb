...
class FieldDailyWeatherTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    @default_crop_params = {name: "test crop", emergence_date: Date.civil(2011, 0o5, 0o1),
  ...
  test "update_balances does something useful" do
    ...
    fdw_first.ref_et = ET
    fdw_first.save!
    puts "saved the first day, it's now #{fdw_first.inspect}"
    $stdout.flush
  ...
  test "can create spreadsheet-identical field" do
    ...
    n_days.times { |day|
      fdw[emergence_day + day].ref_et = ET
      fdw[emergence_day + day].save!
    }
    field.do_balances
  ...
  test "summary has the fields I expect" do
    ...
    [:rain, :irrigation, :deep_drainage, :adj_et].each do |column|
      assert_equal(0.0, summary[column], "#{column} not found in summary  #{summary.inspect}  field: #{field.inspect}")
    end
  ...
  test "summary produces sensible results over a subset of the season" do
    ...
    results.each do |param, result|
      assert_in_delta(result, summary[param], 1.4,
        "#{param} as of #{finish_date} (#{n_fdw} days) should have been #{result} " +
