...
class FieldTest < ActiveSupport::TestCase
  ...
  test "deep drainage is zero for percent cover fields too" do
    pct_pivot = Pivot.all.select { |p| p.farm.et_method.instance_of?(PctCoverEtMethod) }.first
  ...
  def check_field_has_no_lai(field, emergence_date)
    ...
    assert_equal(0.0, fdw_emergence_date.leaf_area_index, "Should start out with nothing for LAI on emergence date of #{emergence_date} for field '#{field.name}'")
  end
  ...
  def setup_field_with_emergence
    ...
    emergence_date = Date.civil(2011, 0o5, 0o1)
  ...
  test "can call mad_adj_et_in_past_week" do
    field, emergence_data = setup_pct_cover_field_with_emergence
    (31..40).each { |ii|
      field.field_daily_weather[ii].ref_et = (0.2 - ii.to_f / 1000.0)
      field.field_daily_weather[ii].entered_pct_cover = ii.to_f
    }
  ...
  test "getting problem ad" do
    ...
    problems = field.problem(Date.new(2011, 0o6, 21))
    assert_not_nil(problems, "Did not find AD problem on test date")
    assert_equal(Date.new(2011, 0o6, 21), problems[problems.keys[0]][0], "AD problem wrong date: test date #{problems[problems.keys[0]][0]}")

    # Test for problem tomorrow
    problems = field.problem(Date.new(2011, 0o6, 20))
    assert_not_nil(problems, "Did not find AD problem on test date + 1")
    assert_equal(Date.new(2011, 0o6, 21), problems[problems.keys[0]][0], "AD problem wrong date: test date + 1 #{problems[problems.keys[0]][0]}")

    # Test for problem in two days
    problems = field.problem(Date.new(2011, 0o6, 19))
    assert_not_nil(problems, "Did not find AD problem on test date + 2")
    assert_equal(Date.new(2011, 0o6, 21), problems[problems.keys[0]][0], "AD problem wrong date: test date + 2 #{problems[problems.keys[0]][0]}")
  end
