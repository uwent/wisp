      ...
      context 'and it has weather station data for the current year' do
        ...
        context 'when weather station data is changed' do
          it 'updates the field daily weather' do
            expect { weather_station_data.update_attribute(rain: 4.0) }.to change { field_daily_weather.reload.rain }.to(4.0)
          end
        end
        ...
        context 'when the weather station data includes percent cover' do
          ...
          it 'propagates to field daily weather' do
            expect { weather_station_data.update_attribute(entered_pct_cover: new_entered_pct_cover) }.to change { field_daily_weather.reload.entered_pct_cover }.to(new_entered_pct_cover)
          end
        end
      end
    end
